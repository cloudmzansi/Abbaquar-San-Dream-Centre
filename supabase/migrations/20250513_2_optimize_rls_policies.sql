...
-- This migration fixes auth_rls_initplan and multiple_permissive_policies issues
...
CREATE INDEX IF NOT EXISTS idx_activities_display_on_rls 
ON activities(display_on) WHERE display_on IN ('home', 'both');

CREATE INDEX IF NOT EXISTS idx_events_rls_support 
ON events(id) WHERE true;

CREATE INDEX IF NOT EXISTS idx_gallery_rls_support 
ON gallery(id) WHERE true;

CREATE INDEX IF NOT EXISTS idx_contact_submissions_rls_support 
ON contact_submissions(id) WHERE true;

CREATE INDEX IF NOT EXISTS idx_profiles_id_rls 
ON profiles(id) WHERE id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_contact_messages_rls_support 
ON contact_messages(id) WHERE true;

CREATE INDEX IF NOT EXISTS idx_donations_rls_support 
